...
var  deleteTask = function(){
        ...
        var re = new RegExp(/^[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|[12][0-9]|3[01])$/);
        task["id"]=id;
        $.post("deleteTask.php",JSON.stringify(task), function(){
            $("#taskrow"+id).remove();    
        });
        drawGanntBars();
  }
...
var addNewRow = function(){
        $('.NewRow').find('input').off('change',addNewRow);
        var id=event.target.id.match(/\d+/)[0];
        var task={}; 
        console.log(id);
        task["taskDescription"]=$('#taskDescription'+id).val();
        ...
        task["completed"]=$('#completed'+id).prop('checked');
        task["comments"]=$('#comments'+id).val();
        task["project"]=$('#projId').html();
        console.log(task);
        $.post("addTask.php",JSON.stringify(task),function(data){
            var resp=JSON.parse(data);
            var newId=resp.NewId;
            console.log(resp);
            $('#taskDescription'+id).attr('id',"taskDescription"+newId);
            $('#startDate'+id).attr('id',"startDate"+newId);
            $('#endDate'+id).attr('id',"endDate"+newId);
            $('#respEmployee'+id).attr('id',"respEmployee"+newId);
            $('#completed'+id).attr('id',"completed"+newId);
            $('#comments'+id).attr('id',"comments"+newId);
            $('#ganntrect'+id).attr('id',"ganntrect"+newId);
            $('#svg'+id).attr('id',"svg"+newId);
            $('#delete'+id).prop("disabled",false);
            $('#delete'+id).attr('id',"delete"+newId);
            $('#taskrow'+id).attr('id',"taskrow"+newId);
            $('#taskDescription'+newId).on('change',saveChangeToTask);
            $('#startDate'+newId).on('change',saveChangeToTask);
            $('#endDate'+newId).on('change',saveChangeToTask);
            $('#respEmployee'+newId).on('change',saveChangeToTask);
            $('#completed'+newId).on('change',saveChangeToTask);
            $('#comments'+newId).on('change',saveChangeToTask);
        });
...
?>
          <form method="post">
              <table>
                <tr>
                   <td>Id:</td>
                   <td id="projId"><?php echo $row["id"];?></td>
                </tr>
                <tr>
                   <td>Client Name:</td>
                ...
                <tr>
                   ...
                   <td><input type="text" id="projectDescription" value="<?php echo $row["projectDescription"];?>"
                        onchange="saveChangeToProject(<?php echo $row["id"];?>)"></td>
                </tr>
                <tr>
...
?>
           ...
           <form id="tasksForm">
              <div style="text-align:right;">
                 <button id="toggleButton" style="width:150px" type="button" onclick="toggleGannt()">View Gannt</button>
              </div>
              ...
              <tr>
                ...
                <th id="ganntHeader">
                    ...
                    <div id="Comments" class="CommentDiv"> 
                         Comments
                    </div>
                </th>
...
?>
                 <tr id="taskrow<?php echo $task["id"];?>">
                   ...
                   <td>
                     <input style="width:100%" class="TaskInput" type="text" id="taskDescription<?php echo $task["id"];?>" value="<?php echo $task["taskDescription"];?>" 
                      onchange=""></td>
                   </td>
                   <td class="fixedWidth">
                     <input type="text" class="dateValidate TaskInput" style="width:100%" id="startDate<?php echo $task["id"];?>" value="<?php echo $task["startDate"];?>" 
                      onchange="drawGanntBars();"></td>
                   </td>
                   <td class="fixedWidth">
                     <input type="text" class="dateValidate TaskInput" style="width:100%"  id="endDate<?php echo $task["id"];?>" value="<?php echo $task["endDate"];?>" 
                      onchange="drawGanntBars();"></td>
                   </td>
                   <td  class="fixedWidth" style="text-align:center">
                     <input type="checkbox" class="TaskInput" id="completed<?php echo $task["id"];?>" <?php if($task["completed"] == 1) echo "checked";?> 
                      onchange="drawGanntBars();"></td>
                   </td>
                   <td class="fixedWidth" style="text-align:center">
                       <select class="TaskInput" style="width:100%" id="respEmployee<?php echo $task["id"];?>" onchange="">
...
?>
                   ...
                   <td>
                       <div id="gannt<?php echo $task["id"];?>" class="gannt GanntDiv" width="100%">
                       ...
                       <div class="CommentDiv" width="100%">
                             <input class="TaskInput" style="width:100%" type="text" id="comments<?php echo $task["id"];?>" value="<?php echo $task["comments"];?>" 
                              onchange=""></td>
                       </div>
                   </td>
                   <td>
                      <button class="DeleteButton" type="button" id="delete<?php echo $task["id"];?>">Delete Task</button>
                   </td>
                 </tr>
...
?>
              ...
              <tr class="NewRow" id="taskrow<?php echo $newRowId;?>">
                <td style="visibility:hidden;width:0px""></td>
                <td>
                     <input style="width:100%" type="text" id="taskDescription<?php echo $newRowId;?>" value="" >
                </td>
                <td  class="fixedWidth">
                     <input style="width:100%" class="dateValidate" onchange="drawGanntBars();" type="text" id="startDate<?php echo $newRowId;?>" value="" >
                </td>
                <td  class="fixedWidth">
                     <input style="width:100%" class="dateValidate" onchange="drawGanntBars();" type="text" id="endDate<?php echo $newRowId;?>" value="" >
                </td>
                <td  class="fixedWidth" style="text-align:center">
                     <input type="checkbox" id="completed<?php echo $newRowId;?>" >
                </td>
                <td  class="fixedWidth">
...
?>
                   ...
                   <td>
                       <div id="gannt<?php echo $newRowId;?>" class="gannt GanntDiv" width="100%">
                       ...
                       <div class="CommentDiv" width="100%">
                             <input style="width:100%" type="text" id="Comments<?php echo $newRowId;?>" value="<?php echo $task["comments"];?>" 
                              onchange=""></td>
                       </div>
                   </td>
                   <td>
                      <button class="DeleteButton" type="button" id="delete<?php echo $newRowId;?>" disabled>Delete Task</button>
                   </td>
              </tr>
...
$(document).ready(function(){
    ...
    $('.TaskInput').on("change",saveChangeToTask);
    $('.DeleteButton').on("click",deleteTask);
    $('.GanntDiv').hide();
    $('.NewRow').find('input').on('change',addNewRow);
});

var toggleGannt=function(){
   $('.GanntDiv').toggle();
   $('.CommentDiv').toggle();
   if ($('.GanntDiv').css('display')=='none')
     $('#toggleButton').html("View Gannt");
   else
     $('#toggleButton').html("View Comments");
}
...
var addNewRowHtml = function(){
   <?php echo "var employees = " . json_encode($empArr) .";"; ?>

  var newRowId = Date.now();
  var newRowText="<tr class=\"NewRow\" id=\"taskrow"+newRowId +"\">"
               + " <td style=\"visibility:hidden;width:0px\"></td>"
               + " <td> <input style=\"width:100%\" type=\"text\" id=\"taskDescription"+newRowId+ "\" ></td>"
               + " <td  class=\"fixedWidth\">"
               + " <input style=\"width:100%\" onchange=\"drawGanntBars();\" type=\"text\" id=\"startDate" + newRowId + "\"  >"
               + " </td>"
               + " <td  class=\"fixedWidth\">"
               + " <input style=\"width:100%\" onchange=\"drawGanntBars();\" type=\"text\" id=\"endDate" + newRowId +"\"  >"
               + " </td>"
               + " <td  class=\"fixedWidth\" style=\"text-align:center\">"
               + " <input type=\"checkbox\" id=\"completed" + newRowId + "\" >"
               + " </td>"
               + " <td  class=\"fixedWidth\">"
               + "     <select style=\"width:100%\" id=\"respEmployee" + newRowId+"\" >";

     employees.forEach(function(emp){
       newRowText +=  "<option value=\""+ emp.id   +"\">" + emp.shortName+ "</option>"; 
     });      
     newRowText += " </select>"
               + " </td>"
               + " <td>"
               + "       <div id=\"gannt" + newRowId + "\" class=\"gannt GanntDiv\" width=\"100%\">"
               + "           <svg id=\"" + newRowId + "\" class=\"ganntSVG\" height=\"20\" width=\"100%\">"
               + "              <rect id=\"ganntrect" + newRowId+ "\" class=\"ganntRect\" x=\"10\" y=\"0\"  height=\"20\" width=\"300\">"
               + "              </rect>"
               + "              <line class=\"today\" x1=\"10\" x2=\"10\" y1=\"0\" y2=\"20\" stroke=\"red\"></line>"
               + "           </svg>"
               + "        </div>"
               + "        <div id=\"comment" + newRowId + "\" class=\"CommentDiv\" width=\"100%\">"
               + "              <input style=\"width:100%\" type=\"text\" id=\"Comments" + newRowId + "\">"
               + "  </td> </div> </td> "
               + " <td>"
               + "       <button class=\"DeleteButton\" type=\"button\" id=\"delete" + newRowId + "\" disabled>Delete Task</button>"
               + " </td>"
               + "</tr>";
     $('.NewRow td input').on("input", addNewRow);
     $('.NewRow td textarea').on("input", addNewRow);
     $('#tasksTable tbody').append(newRowText);
     $('.NewRow').find('input').on('change',addNewRow);
     $('#delete' + newRowId).on('click',deleteTask);
     if ($('.GanntDiv').css('display')=='none')
        $('#gannt'+newRowId).css('display','none');
     else
        $('#comment'+newRowId).css('display','none');
     drawGanntBars();
}
</script>
